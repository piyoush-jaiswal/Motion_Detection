...
src = "http://162.16.2.142:4747/video"
#start the webcam
...
def Alert(frame):
    ...
    prepare_and_send_email('it20034@glbitm.ac.in',
                            'it20034@glbitm.ac.in',
                            'Alert Alert Alert',
    ...
    while True:
        ...
        if success:
            ...
            if threshold.sum() > 1000:    #threshold.sum() is the sum of the pixels 
                if count_frame <5:
                    count_frame += 1
                elif count_frame == 5 and next_send_mail == True:
                    frame = cv2.putText(frame, "STATUS: {}".format('MOTION DETECTED'), (10, 60), cv2.FONT_HERSHEY_SIMPLEX,
